...
df_stock_codes = pd.DataFrame(pd.read_csv(stock_data_path + var_date +'_N_close_output.csv', index_col=None))

existCode_array = []

while True:

    print("---横盘_过高_3日_收盘过左边最高收盘价-----------")
    # 循环抽出的股票代码
    loop_index = 0
    for stock_code in df_stock_codes.code:
        ...
        try:
            ...
            if ( 1 == 1
                # T 型
                and float(price_today) >= float(high_today)*0.992
                # 当日上涨
                and float(price_today) >= float(open_today)
                # 接近左边最高收盘价
                and float(price_today) >= float(left_max_close_value) * 0.992
                ):
