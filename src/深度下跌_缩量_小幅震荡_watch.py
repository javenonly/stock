...
df_stock_codes = pd.DataFrame(pd.read_csv(stock_data_path + var_date +'_L_search.csv', index_col=None))

existCode_array = []

print("---《最近n天金叉》，缩量下跌，今日下跌至ma24-----------")
print_loop = 0
while True:
    print(".",end=" ")
    if (print_loop % 20 == 0):
    ...
    for stock_code in df_stock_codes.code:
        ...
        min_change_value = df_stock_codes.iloc[loop_index].min_change_value
        loop_index += 1
        ...
        try:
            ...
            if ( 1 == 1
                ...
                # # 今日下跌至ma24
                and (float(high_today) - float(low_today)) <= float(min_change_value)*0.65
                ):
