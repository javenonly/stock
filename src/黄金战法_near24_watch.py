...
df_stock_codes = pd.DataFrame(pd.read_csv(stock_data_path + var_date +'_A_search_near24.csv', index_col=None))
    ...
    for stock_code in df_stock_codes.code:
        ...
        try:
            ...
            if (
                ...
                # # 当日上涨
                float(price_today) >= float(ma5_value)
                # # 今日下跌至ma24
                or float(low_today) >= float(low_value)
                ):
