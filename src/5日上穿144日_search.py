...
#先生成一个文件
out = open(stock_data_path + first_day + '_5_144_search.csv','a', newline='')
csv_write = csv.writer(out,dialect='excel')
# ,code,max_high_value(最高价)
csv_write.writerow(['',"code","ma144"])
# 遍历所有股票
for stock_code in df_all_code.code:
    ...
    try:
        ...
        if ( data1_ma5 > data1_ma144 and data1_ma144 > data1_ma24):

            below_index = 0
            for index in range(int_scope):
                if df_history.iloc[index].ma5 < df_history.iloc[index].ma144:
                    below_index = index
                    break
