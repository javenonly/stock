...
for item_code in df1.code:
    ...
    try:
        ...
        if ((df.iloc[long_date_index].volume > df.iloc[long_date_index + 1].volume * volume_up_rate)  # 长阳日的成交量 > 1天
            and (df.iloc[long_date_index].volume > df.iloc[long_date_index + 2].volume * volume_up_rate)  # 长阳日的成交量 > 2天
            and (df.iloc[long_date_index].volume > df.iloc[long_date_index + 3].volume * volume_up_rate)  # 长阳日的成交量 > 3天
            # and (df.iloc[long_date_index].volume > df.iloc[long_date_index + 4].volume * volume_up_rate)  # 长阳日的成交量 > 4天
            # and (df.iloc[long_date_index].volume > df.iloc[long_date_index + 5].volume * volume_up_rate)  # 长阳日的成交量 > 5天
            # 2.长阳 p_change > up_price_low_p && p_change < up_price_high_p
