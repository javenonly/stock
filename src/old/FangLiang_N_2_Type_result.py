    ...
    try:
        ...
        for item_date in df.date:
            ...
            if (
                ...
                # 长阳日+2天下跌
                and (df.iloc[long_date_index - 2].high < df.iloc[long_date_index].high)
                # 长阳日+3天过高
                and (df.iloc[long_date_index - 3].high > df.iloc[long_date_index].high)
                # 长阳日+2天收阳线
                # and (df.iloc[long_date_index - 2].close > df.iloc[long_date_index - 2].open)
                ):
