...
# ,code
#直接保存
out = open(stock_data_path + var_date + '_L_down_n_output.csv','a', newline='')
...
for stock_code in df_all_code.code:
    ...
    try:
        ...
        #从第一条数据开始
        right_index = -1
        data_right_1 = df_history.iloc[right_index+1]
        data_right_2 = df_history.iloc[right_index+2]
        data_right_3 = df_history.iloc[right_index+3]
        data_right_4 = df_history.iloc[right_index+4]
        # 最高价集合
        right_data_close_array = [data_right_1.close,data_right_2.close,data_right_3.close,data_right_4.close]
        right_max_close_value = max(right_data_close_array)
        ...
        if (
            ...
            # 最高日
            and df_history.iloc[most_high_index+4].open > df_history.iloc[most_high_index+4].close
            and df_history.iloc[most_high_index+4].high < df_history.iloc[most_high_index+4].open * 1.003
            # and df_history.iloc[most_high_index].close <= df_history.iloc[most_high_index].low * 1.003
            ):
                # 左边长度
                ...
                if right_max_close_value >= left_max_high_value :
                    print("%06d"%stock_code)
