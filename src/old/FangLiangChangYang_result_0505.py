    ...
    try:
        ...
        for item_date in df.date:
            ...
            if (
                ...
                # 长阳日上影线---->短
                and ((df.iloc[long_date_index].high - df.iloc[long_date_index].close) / (df.iloc[long_date_index].high - df.iloc[long_date_index].low) > 0.28)
                ...
                # 长阳日+1天与长阳日有交集
                # and (df.iloc[long_date_index - 1].low < df.iloc[long_date_index].high)
                # 长阳日+1天低开
                and (df.iloc[long_date_index - 1].open < df.iloc[long_date_index].close)
                # 长阳日+1天上影线短
                #and ((df.iloc[long_date_index - 1].high - df.iloc[long_date_index - 1].close) / (df.iloc[long_date_index - 1].high - df.iloc[long_date_index - 1].low) < 0.29)
                ):
