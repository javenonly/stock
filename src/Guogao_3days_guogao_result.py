...
for item_code in df_all_code.code:
    ...
    try:
        ...
        for item_date in df_history.date:
            ...
            max_value = max(front_5.high,front_6.high,front_7.high,front_8.high,front_9.high)
            if (
                #■■■■■■■■■■■■■■■■■■■【选股条件】■■■■■■■■■■■■■■■■■■■■■■
                #涨
                (front_4.p_change > 0 ) 
                #过前高，> max_value
                and (front_4.high > max_value)
                # 过高日的前一日 < max_value
                and (front_5.high < max_value)
                # 过高日后，跌2天
                and (front_3.high < front_4.high)
                and (front_2.high < front_4.high)
                and (front_1.high < front_4.high)
                #■■■■■■■■■■■■■■■■【附加条件】■■■■■■■■■■■■■■■■■■■■■■■■■■■■■
                # （买入日，观望日）又过前高
                # 实时选股条件
                and (buy_day.high > front_4.high)
                ...
                for up_day in up_days:
                    if (df_history.iloc[buy_index - up_day].high / front_4.high > price_up_rate):
                        up_cnt_boolean = True
