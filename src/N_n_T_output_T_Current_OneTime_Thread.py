...
def print_up_stock( stock_code , data_1_close):
    try:
        # 获取股票实时数据
...
df_stock_codes = pd.DataFrame(pd.read_csv(stock_data_path + var_date +'_N_n_T_output.csv', index_col=None))
...
for stock_code in df_stock_codes.code:
    data_1_close = df_stock_codes.iloc[loop_index].data_1_close
    loop_index += 1
    try:
        threads.append(threading.Thread(target=print_up_stock,args=(stock_code,data_1_close,)))
    except:
