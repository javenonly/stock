...
#先生成一个文件
out = open(stock_data_path + first_day + '_5_99_before_search.csv','a', newline='')
csv_write = csv.writer(out,dialect='excel')
# ,code,max_high_value(最高价)
csv_write.writerow(['',"code","ma5","ma99"])
# 遍历所有股票
for stock_code in df_all_code.code:
    ...
    try:
        ...
        # 第二条数据ma5
        data2_ma5 = df_history.iloc[1].ma5
        # 第一条数据ma24
        ...
        # 差
        deviation_5 = data1_ma5 - data2_ma5
        deviation_24 = data1_ma24 - data2_ma24
        deviation_99 = data1_ma99 - data2_ma99
        # 第一条数据ma144
        data1_ma144 = df_history.iloc[0].ma144

        if ( data1_ma99 > data1_ma5 
        and data1_ma5 > data1_ma24 
        and deviation_5 > 0 
        and deviation_24 > 0 
        and (data1_ma5 + deviation_5*3) >= (data1_ma99 + deviation_99*3)
        and data1_ma99 / data1_ma24 < 1.08
        and data1_ma99 / data1_ma24 > 1.03
        and data1_close / data1_ma24 < 1.08):
            # index_down = index
            print("%06d"%stock_code)
            csv_write.writerow([index_stock,"%06d"%stock_code,data1_ma5,data1_ma99])
            index_stock += 1
