...
for item_code in df_all_code.code:
    ...
    try:
        ...
        for item_date in df_history.date:
            ...
            # 涨 > 前高（前几日的最高价 max_value）
            front_1 = df_history.iloc[buy_index+1]
            # 跌 < 前高（前几日的最高价 max_value）
            ...
            if (
                ...
                # T型 
                and ((buy_day.high - buy_day.close) / (buy_day.high - buy_day.low) < 0.25)
                # 收盘价 > 前一日的中线价格之上（ (high + low) / 2）
                and (buy_day.close >= (front_1.high + front_1.low)/2)
                # 收盘价 > 前一日的收盘价
                # and (buy_day.close >= front_1.close)
                #▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲尾盘(★★★★2.30以后★★★★)选股条件▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲
                ):
                #■■■■■■■■■■■■■■■■【计算】■■■■■■■■■■■■■■■■■■■■■■■■■■■■■
                # 符合选股条件的总数
